...
//A decent level of programming knowledge is assumed. Comments are based on that assumption, as I don't go into details for every line.
// I just give an overview of what is happening in each function. 
    ...
    //there will be some weird math with these later. there was a requirement where we had to be able to "change the date" to see if our fines system worked.
    ...
    It takes the passed in s, and splits it at each \n (newline character)
    if you chose the first option, i.e., you want to get the ISBN13s out of s, then it will loop through each line (from the split),
    and add those to the ArrayList, and finally return it.
    If you chose the second option, i.e., you want everything other than an ISBN13, then it will loop through each line (from the split),
    and add those lines to the ArrayList, and finally return it.
    */
    ...
    //This function takes an Object array, and populates it as a row into the table you passed in.
    private void PopulateTable(Object res[], JTable table) {
        DefaultTableModel model = (DefaultTableModel) table.getModel();
        model.addRow(res);
    }

    //when the user enters info in the search area, if there is some non-whitespace character there, then enable the search button
    ...
    //when the user clicks the exit option to end the program
    private void ExitButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ExitButtonActionPerformed
        System.exit(0);
    }//GEN-LAST:event_ExitButtonActionPerformed

    //This function adds a borrower. The code is broken into sections with comments to explain what's going on
    private void AddBorrowerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_AddBorrowerButtonActionPerformed
        ...
        //Start Section 1: Get the actual state text with a little formatting
        ...
        /*
        Start Section 2: if phone_filled is true, run the first sqlQuery, which has the user's SSN, first&last name, address, and phone number
        else, do the same thing without adding a phone number
        */
        String sqlQuery = "";
        int res = -1;
        if (phone_filled) {
        ...
        //Start Section 3: execute the sqlQuery on the database, show a corresponding message to whether it fails or succeeds, and add the borrower to the BorrowerTable
    ...
    /*
    when the user enters a character into the phone number field,
    if the phone number field isn't empty (not only whitespace) and it's a valid phone number format, then set phone_b to true and phone_filled to true
    if not, then check if it's empty, and it's not a valid phone number format, if so, then set phone_b to true and phone_filled to false
    else, set phone_b to false and disable the AddBorrowerButton
    at the end, check if the other bools are true, along with phone_b, and if so, enable the AddBorrowerButton
    */
    ...
    /*
    when the user changes the state from the state dropdown menu, check if it's a valid state option. 
    if so, then set state_b to true.
    at the end, check if the other bools are true, along with state_b, and if so, enable the AddBorrowerButton
    */
    ...
    /*
    when the user enters a character into the city field,
    if the city field isn't empty (not only whitespace), then set city_b to true 
    else, set city_b to false and disable the AddBorrowerButton
    at the end, check if the other bools are true, along with city_b, and if so, enable the AddBorrowerButton
    */
    ...
    /*
    when the user enters a character into the name field,
    if the name field isn't empty (not only whitespace), then set name_b to true 
    else, set name_b to false and disable the AddBorrowerButton
    at the end, check if the other bools are true, along with name_b, and if so, enable the AddBorrowerButton
    */
    ...
    /*
    when the user enters a character into the street field,
    if the street field isn't empty (not only whitespace), then set street_b to true 
    else, set street_b to false and disable the AddBorrowerButton
    at the end, check if the other bools are true, along with street_b, and if so, enable the AddBorrowerButton
    */
    ...
    /*
    when the user enters a character into the SSN field,
    if the SSN field isn't empty (not only whitespace) and it's a valid SSN format, then set ssn_b to true
    else, set ssn_b to false and disable the AddBorrowerButton
    at the end, check if the other bools are true, along with ssn_b, and if so, enable the AddBorrowerButton
    */
    ...
    /*
    when the user clicks an entry in the SearchTable, make sure they only clicked one. if not, show a corresponding message.
    if they only clicked one entry, check if it's available based on the availability column value (should be current)
    */
    ...
    //does the exact same thing as SearchTableMouseClicked(). just called if the mouse isn't completely still when the user clicks.
    ...
    //when the user clicks checkout on the search tab (if it's enabled), it will switch to the search tab and "auto-populate" the book title and ISBN13 there.
    ...
    private void CheckOut_COMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_CheckOut_COMouseClicked
        //Start Section 1: mostly just variable initialization for later, and getting the chosen Card_id
        ...
        //Start Section 2: sqlQuery is set and is executed on the database. if the date_in field isn't null, then the book isn't available. else, it's available
        ...
        /*
        Start Section 3: If it's available, there's an ISBN13, a card ID was chosen, and the checkout button is enabled, then check to see if that card ID is present in the database.
        If it is, then check to see how many books they have checked out. (There is a max of 3 books.)
        */
        if (available) {
            if (!isbn.isBlank() && cardID > 0 && CheckOut_CO.isEnabled()) {
                ...
                /*
                Start Section 4: If the card ID is valid, and they have less than 3 books out, insert a new record into the book_loans table.
                Show a corresponding message depending on whether it fails or succeeds.
                reset the fields for checking out, and set some meaningful information in the check out details
                */
                if (validID) {
                    if (loans < 3) {
                        ...
                        /*
                        Start Section 5: Create a row in the fines table to keep track of the borrower's fines for all books.
                        Show a corresponding message depending on whether it fails or succeeds.
                        */
                        ...
                        //The borrower has 3 books out already.
                    } else {
                        JOptionPane.showConfirmDialog(null,
        ...
        //user tried to check out a book that wasn't available
        else {
            JOptionPane.showConfirmDialog(null,
    ...
    /*when the user changes the value of the CardID spinner.
    if the new value is greater than 0, set id_out to true and the Checkout button to true if both title_out and isbn_out are true 
    else, disable the checkout button
    */
    ...
    /*when the user changes the value of the CardID spinner.
    if the new value is greater than 0, set id_in to true
    else, set id_in to false.
    then enable the getResultsButton if either id_in, name_in, or isbn_in is true.
    */
    ...
    //when the user clicks the getResultsButton. done in sections.
    private void getResultsButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_getResultsButtonMouseClicked
        //Start Section 1: mostly just variable initialization for later, and getting the name, isbn, and id.
        ...
        /*Start Section 2: 
        if the name isn't blank, get the id from the resulting row of the sqlQuery.
        using that id, get any books loans that borrower has in sqlQuery2
        using the isbn13 from that result, get the title of the book in sqlQuery3.
        create a row for the table with mostly the info from sqlQuery2.
        */
        if (!name.isBlank()) {
        ...
        /*
        Start Section 3: if the user has entered an ISBN13, then run the sqlQuery to get cardID, book title, DateIn, DateOut, and DueDate.
        create a row in the table with that info.
        */
        if (isbn_in) {
        ...
        /*
        Start Section 3: if the user has entered an id, then run the sqlQuery to get ISBN13, book title, DateIn, DateOut, and DueDate.
        create a row in the table with that info.
        */
        if (id_in) {
    ...
    /*
    when the user clicks an entry in the CheckInTable, make sure they only clicked one. if not, show a corresponding message and disable the CheckInButton
    if they only clicked one entry, enable the CheckInButton
    */
    ...
    //does the exact same thing as CheckInTableMouseClicked(). just called if the mouse isn't completely still when the user clicks.
    ...
    private void CheckInButtonMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_CheckInButtonMouseClicked
        ...
        /*
        Start Section 1: get the ISBN, ID, and weird date. Some variable initialization
        */
        ...
        /*
        Start Section 2: if the CheckInButton is enabled, set the date_in to the date from above for the corresponding entry in book_loans.
        Show a corresponding message depending on whether it fails or succeeds.
        */
        if (CheckInButton.isEnabled()) {
            ...
            try (Statement s1 = connection.createStatement()) {
                ...
                if (result == 0) {
                    ...
                    /*
                    Start Section 3: if the book was turned in late, add (daysLate*.25) cents to the corresponding fines table entry and set the date_in to the date from above
                    Show a corresponding message depending on whether it fails or succeeds.
                    */
                    ...
                    try (Statement s2 = connection.createStatement()) {
                        ...
                        while (rs2.next()) {
                            ...
                            if (days != 0) {
                                ...
                                /*
                                Start Section 4: If it's not late, set the date_in to be the date from above.
                                Show a corresponding message depending on whether it fails or succeeds.
                                */
                            } else {
    ...
    */
    ...
    /*
    when the user enters a character into the name field,
    if the name field isn't empty (not only whitespace), then set name_in to true 
    else, set name_in to false
    then enable the getResultsButton if either id_in, name_in, or isbn_in is true.
    */
    ...
    /*
    when the user enters a character into the isbn field,
    if the isbn field isn't empty (not only whitespace) and it's a valid format, then set isbn_in to true 
    else, set isbn_in to false
    then enable the getResultsButton if either id_in, name_in, or isbn_in is true.
    */
                ...
                for (String s : isbn) {
                    ...
                    if (!ret_isbn.contains(s)) { //if it doesn't have the current isbn, then do the rest. Otherwise, don't so there isn't dups in the table.
                        ...
                        try (Statement s1 = connection.createStatement()) { //create a statement
                            ...
                            e.printStackTrace(); //just print the usually cryptic stuff that it says happened
                        }
    ...
    private void RefreshItemActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_RefreshItemActionPerformed
        /*
        Section 1: Add 1 day to TODAY, and variable initialization
        */
        ...
        /*
        Start Section 2: get all book_loans entries, if they're late (due_date has passed) and the fines haven't been paid, update each row's fines to be (numDaysLate*.25).
        Show a corresponding message depending on whether it fails or succeeds.
        */
    ...
    /*
    this function will just take the selected cardID, use it to index the book_loans table, then use the Loan_id from book_loans as an index into fines.
    Once there, it will set paid to be true.
    It also shows a corresponding message depending on whether it fails or succeeds
    */
    ...
    //add 1 day to the TODAY (part of the instructions at line 35)
    ...
    /*
    when the user clicks an entry in the finestable, make sure they only clicked one row.
    if so, if its paid column value is false, enable the PayFinesButton
    if not, show a corresponding message and disable the PayFinesButton
    if they clicked more than one row, diable the PayFinesButton and show a corresponding message
    */
    ...
    //does the exact same thing as finestableMouseClicked(). just called if the mouse isn't completely still when the user clicks.
    ...
    private void RefreshFinesTable() {
        //Start Section 1: variable initialization
        ...
        /*
        Start Section 2: get all books_loans entries and order them from least to greatest.
        for the first iteration:
        get the cardID and loan_id
        using the loan_id as an index into the fines table, if it's not paid, add the fine amount to fines, and set paid.
        from the second iteration and on:
        if card_id IS NOT equal to the sqlQuery's resulting card_id, create a row for the finestable with the card_id, fines, and paid values.
        after, do the same thing as in the beginning: 
        get the cardID and loan_id
        using the loan_id as an index into the fines table, if it's not paid, add the fine amount to fines, and set paid.
        if the card_id IS equal to the sqlQuery's resulting card_id, just keep adding to the fines.
        At the end, create a row for the finestable with the card_id, fines, and paid values.
        */
    ...
    //Just for looks. Centers the tables.
    ...
    /*
    Populate the BorrowerTable with the information directly from the database.
    Clears the table first, then for each row it finds, it will set the rowData array with that information, and add it to the table.
    */
